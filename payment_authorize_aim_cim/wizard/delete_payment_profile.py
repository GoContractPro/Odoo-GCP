...
class delete_payment_profile(osv.TransientModel):
    ...
    def del_pay_profile(self, cr, uid, ids, context=None):
        ...
        if Trans_key and Login_id:
            ...
            edit_profile_id = self.pool.get('edit.payment.profile').search(cr,uid,[('payment_profile_id','=',data.payment_profile_id.id)])
            if edit_profile_id:
                self.pool.get('edit.payment.profile').unlink(cr, uid, edit_profile_id)
