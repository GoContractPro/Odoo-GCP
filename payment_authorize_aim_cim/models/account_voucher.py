...
class AccountVoucher(osv.Model):
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default.update(vals)
        return super(AccountVoucher, self).copy(cr, uid, id, default=default, context=context)
...
#        self.do_transaction(cr, uid, ids, refund=True,context=context)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('cc_number', False):
            ...
            del res['enc_value']
            vals.update(res)
        result = super(AccountVoucher, self).create(cr, uid, vals, context)
        return result
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        if fields and 'cc_number' in fields and 'key' not in fields:
            fields.append('key')
        vals = super(AccountVoucher, self).read(cr, uid, ids, fields, context, load)
...
#             payment_profile_id = cust_pay_profile_ids and cust_pay_profile_ids[0] or False
#             if res.get('value'):
#                 res.get('value')['payment_profile_id'] = payment_profile_id
#         if not res.has_key('value'):
#             res['value'] = {}
#         cc_allow_refunds = jorurnal_cc_allow = False
